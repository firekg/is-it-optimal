...
class ActiveLearning:
      ...
      def Set(self, user_hypo):
            ...
            self.num_hypo, self.num_feature, self.num_label, self.p_teacher_x_h, self.p_teacher_xy_h, self.p_learner_h_xy, self.p_y_xh, self.delta_g_h, self.phx = Init.Set(user_hypo, None,
                                                                                                                                                                            self.knowledge)

      def P_Task(self):
            # Get a new knowledgeability table
            knowledgeability = Task.NKnowledgeability_Task(self.hypo_table, self.num_hypo, self.num_feature, self.num_label, 1 / 4)

            for n in range(len(knowledgeability)):
                  print(knowledgeability[n])
                  p, s = Task.Probability_Task(self.hypo_table, self.num_hypo, self.num_feature, self.num_label, copy.deepcopy(knowledgeability[n]))
                  print(p)
