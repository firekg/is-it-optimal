...
def Knowledgeability_Task(hypo, feature, label, p_teacher_xy_h, p_teacher_x_h, p_y_xh, delta_g_h, phx, num_iteration):
    ...
    Teach.K_PTeacher_xh(hypo, feature, label, p_teacher_xy_h,
                        p_teacher_x_h, p_learner_h_xy, delta_g_h)
    ...
    while True:
        ...
        Learn.K_PLearner_h_xy(hypo, feature, label, p_y_xh,
                              p_learner_h_xy, p_teacher_x_h, phx)
        ...
        if loop % 10 == 0:
            ...
            if numpy.array_equal(p_teacher_x_h, temp_p_teacher_x_h):
                break

        # End the loop by checking the number of iterations
        if loop > num_iteration:
            break
...
# [0,0,a,a]
# [0,0,a,a]
def Pattern_Cube_K(number_hypo):
    K_set = []
    # The size should be at least 1
    # If the cube size = 1, then it is an Identity matrix
    cube = 1

    # The original Knowledgeability matrix K
    K_Zeros = numpy.zeros((number_hypo, number_hypo), dtype=float)
    while cube <= number_hypo:
        ...
        while s > 0:
            if(s >= cube):
                Lst_Original.append(cube)
            else:
                uni = False
                Lst_Original.append(s)
            s -= cube

        mx = len(Lst_Original) - 1
        Lst = []

        if uni == False:
            while mx >= 0:
                Lst.append(copy.deepcopy(Lst_Original))
                temp = Lst_Original[mx]
                Lst_Original[mx] = Lst_Original[mx-1]
                Lst_Original[mx-1] = temp
                mx -= 1
        else:
            Lst.append(Lst_Original)

        for L in Lst:
            K = copy.deepcopy(K_Zeros)

            a = 0
            # If there is enough place for the next cube then use the cube size
            # else, use the remaing space size
            for elements in L:
                p = round(1 / elements, 5)
                K[a:a + elements, a:a + elements] = p
                a += elements
...
def Sample_P(hypo_table, number_hypo, number_feature, number_label, k_matrix, iter):
    ...
    for hypo_idx in range(hypo_table_size):
        ...
        # Get the PT
        p_learner_h_xy = Knowledgeability_Task(
            num_hypo, num_feature, num_label, p_teacher_xy_h, p_teacher_x_h, p_y_xh, k_matrix, phx, iter)
        ...
        prob_find, true_label = Observe.Observe(
